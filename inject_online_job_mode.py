...
<section id="online-dashboard" class="online-dashboard" aria-live="polite">
  <section class="method online-method" aria-live="polite"><div><div class="eyebrow">ТЕКУЩИЕ ВАКАНСИИ</div><div class="method-region" id="online-method-region">Выберите регион</div></div><div><div class="method-title" id="online-method-title">Насколько легко найти вакансию с нормальной для региона зарплатой</div><div class="method-text" id="online-method-text">Оценка учитывает число объявлений на каждого работающего или ищущего работу и сравнивает предлагаемую зарплату с типичной зарплатой региона.</div></div></section>
...
function setDataMode(mode){const online=mode==="online";document.body.classList.toggle("online-mode",online);modeOfficial.classList.toggle("active",!online);modeOnline.classList.toggle("active",online);modeOfficial.setAttribute("aria-pressed",!online);modeOnline.setAttribute("aria-pressed",online);pageTitle.textContent=online?'Где проще найти вакансию с типичной для региона зарплатой':officialTitle;pageSubtitle.textContent=online?'Оперативный срез объявлений hh.ru и SuperJob на 13 сентября 2026 года. 1 — сравнительно легче, 10 — труднее. Наведите курсор на субъект или щёлкните, чтобы закрепить панель.':officialSubtitle;if(online)drawOnlineDashboard();}
...
function renderOnlinePanel(iso){const d=onlineRegionData[iso];if(!d){onlineMethodRegion.textContent='Для региона нет данных';onlineMethodTitle.textContent='Оценка онлайн-вакансий не рассчитана';onlineMethodText.textContent='Отсутствие данных не означает, что искать работу здесь легче или труднее.';onlinePanel.innerHTML='<div class="eyebrow">Нет данных онлайн-сервисов</div><div class="region-name">Для региона нет сопоставимых объявлений</div><p class="online-note">Отсутствие данных не означает хорошие или плохие условия поиска работы.</p>';return;}onlineSelected=iso;onlineMethodRegion.textContent=`${d.region} · ${d.score.toFixed(1)} из 10`;onlineMethodTitle.textContent=d.score>=7?'Вакансию с нормальной для региона зарплатой найти сравнительно трудно':d.score>=4?'Средние условия поиска среди регионов':'Вакансию с нормальной для региона зарплатой найти сравнительно легче';onlineMethodText.textContent=`На 10 тыс. работающих или ищущих работу приходится ${d.listings_per_10k.toFixed(1)} объявления; типичная предлагаемая зарплата — ${d.median_salary?onlineFmt.format(d.median_salary)+' ₽':'не указана'}.`;document.querySelectorAll('.online-region').forEach(x=>x.classList.toggle('selected',x.dataset.iso===iso));onlinePanel.innerHTML=`<div class="eyebrow">ВАКАНСИИ · ${onlineEsc(d.snapshot_at.slice(0,10))}</div><div class="region-name">${onlineEsc(d.region)}</div><div class="overall"><span>Насколько трудно найти вакансию с нормальной зарплатой<br><small>1 — условия лучше, 10 — хуже</small></span><strong style="color:${onlineColor(d.score)}">${d.score.toFixed(1)}</strong></div><div class="online-metrics"><div><span>Объявлений на площадках</span><strong>${onlineFmt.format(d.reported_total)}</strong></div><div><span>На 10 тыс. работающих или ищущих работу</span><strong>${d.listings_per_10k.toFixed(1)}</strong></div><div><span>Типичная зарплата в объявлениях</span><strong>${d.median_salary?onlineFmt.format(d.median_salary)+' ₽':'нет данных'}</strong></div><div><span>Объявления с указанной зарплатой</span><strong>${d.salary_coverage.toFixed(0)}% выборки</strong></div></div><h3 class="rank-title" style="margin-top:15px">Направления с наименьшим выбором вакансий</h3><p class="online-note">Маленькие выборки скорректированы, чтобы единичное объявление не искажало рейтинг.</p>${d.groups.slice(0,9).map(g=>`<div class="online-rank"><b>${g.rank}</b><button class="rank-name" data-online-group="${onlineEsc(g.name)}">${onlineEsc(g.name)}<small>${g.sample} объявлений в выборке · типичная зарплата ${g.median_salary?onlineFmt.format(g.median_salary)+' ₽':'не указана'}</small></button><span class="badge" style="background:${onlineColor(g.score)}">${g.score.toFixed(1)}</span></div><div class="online-example" data-online-detail="${onlineEsc(g.name)}" hidden>${g.examples.map(v=>`<a href="${onlineEsc(v.url)}" target="_blank" rel="noopener">${onlineEsc(v.title)}</a>${v.salary_mid_rur?' · '+onlineFmt.format(v.salary_mid_rur)+' ₽':''}`).join('<br>')||'Нет примеров в выборке'}</div>`).join('')}`;onlinePanel.querySelectorAll('[data-online-group]').forEach(b=>b.onclick=()=>{const x=onlinePanel.querySelector(`[data-online-detail="${CSS.escape(b.dataset.onlineGroup)}"]`);x.hidden=!x.hidden;});}
