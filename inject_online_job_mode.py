...
def prepare_online_data():
    ...
    for source, frame in raw_status.groupby("source", sort=False):
        ...
        if source == "hh.ru":
            message = f"получено: {ok} регионов; без сопоставления: {len(frame) - ok}"
            state = "ok" if ok else "недоступно"
        else:
            message = str(frame.iloc[0]["status"])
            state = "ok" if message == "ok" else "ограничено"
...
<section id="online-dashboard" class="online-dashboard" aria-live="polite">
  <h2 class="section-title">Актуальные объявления сервисов поиска работы</h2>
  <p class="section-lead">Это отдельный оперативный срез объявлений, а не статистика всего рынка труда. Индекс 1–10 показывает, где сравнительно труднее найти объявление с приемлемой оплатой: 65% — сколько объявлений приходится на рабочую силу региона, 35% — насколько зарплата выборки объявлений соотносится с официальной медианной зарплатой региона.</p>
  <div class="source-status" id="online-source-status"></div>
  ...
  <section class="online-section"><h3>Как читать этот режим</h3><p class="online-note">Высокий балл означает сочетание малого числа активных объявлений относительно рабочей силы и более слабой оплаты в доступной зарплатной выборке. Балл не измеряет число откликов, скрытые вакансии или вероятность оффера. Карточки первых двух страниц поиска используются для структуры профессий и зарплат; общее число результатов берётся из счётчика площадки. Поэтому рейтинг профессий — диагностическая выборка, а не перепись всех объявлений.</p><div class="online-downloads"><a href="online_job_vacancies_combined.csv" download>Объединённые вакансии CSV</a><a href="online_job_market_by_region.csv" download>Регионы CSV</a><a href="online_job_market_by_region_and_group.csv" download>Профгруппы CSV</a><a href="online_job_sources_status.csv" download>Статус источников CSV</a></div></section>
</section>
'''
...
function setDataMode(mode){const online=mode==="online";document.body.classList.toggle("online-mode",online);modeOfficial.classList.toggle("active",!online);modeOnline.classList.toggle("active",online);modeOfficial.setAttribute("aria-pressed",!online);modeOnline.setAttribute("aria-pressed",online);pageTitle.textContent=online?'Где проще найти подходящую вакансию на онлайн-площадках':officialTitle;pageSubtitle.textContent=online?'Оперативный срез объявлений на 13 сентября 2026 года. Это данные площадок, а не официальная статистика занятости; серые регионы не имеют сопоставимого онлайн-среза.':officialSubtitle;if(online)drawOnlineDashboard();}
...
function renderOnlinePanel(iso){const d=onlineRegionData[iso];if(!d){onlinePanel.innerHTML='<div class="eyebrow">Нет данных онлайн-сервисов</div><div class="region-name">Для региона нет сопоставимого среза</div><p class="online-note">Регион остаётся на карте серым. Это не означает лёгкость или сложность поиска.</p>';return;}onlineSelected=iso;document.querySelectorAll('.online-region').forEach(x=>x.classList.toggle('selected',x.dataset.iso===iso));onlinePanel.innerHTML=`<div class="eyebrow">ОНЛАЙН-ОБЪЯВЛЕНИЯ · ${onlineEsc(d.snapshot_at.slice(0,10))}</div><div class="region-name">${onlineEsc(d.region)}</div><div class="overall"><span>Сложность найти приемлемое объявление</span><strong style="color:${onlineColor(d.score)}">${d.score.toFixed(1)}</strong></div><div class="online-metrics"><div><span>Найдено объявлений на площадках</span><strong>${onlineFmt.format(d.reported_total)}</strong></div><div><span>На 10 тыс. рабочей силы</span><strong>${d.listings_per_10k.toFixed(1)}</strong></div><div><span>Медиана зарплат выборки</span><strong>${d.median_salary?onlineFmt.format(d.median_salary)+' ₽':'нет данных'}</strong></div><div><span>Зарплата указана</span><strong>${d.salary_coverage.toFixed(0)}% выборки</strong></div></div><h3 class="rank-title" style="margin-top:15px">Группы, где выбор предложений слабее</h3><p class="online-note">Рейтинг основан на выборке карточек; малые группы сглажены.</p>${d.groups.slice(0,9).map(g=>`<div class="online-rank"><b>${g.rank}</b><button class="rank-name" data-online-group="${onlineEsc(g.name)}">${onlineEsc(g.name)}<small>${g.sample} карточек · медиана ${g.median_salary?onlineFmt.format(g.median_salary)+' ₽':'не указана'}</small></button><span class="badge" style="background:${onlineColor(g.score)}">${g.score.toFixed(1)}</span></div><div class="online-example" data-online-detail="${onlineEsc(g.name)}" hidden>${g.examples.map(v=>`<a href="${onlineEsc(v.url)}" target="_blank" rel="noopener">${onlineEsc(v.title)}</a>${v.salary_mid_rur?' · '+onlineFmt.format(v.salary_mid_rur)+' ₽':''}`).join('<br>')||'Нет примеров в выборке'}</div>`).join('')}`;onlinePanel.querySelectorAll('[data-online-group]').forEach(b=>b.onclick=()=>{const x=onlinePanel.querySelector(`[data-online-detail="${CSS.escape(b.dataset.onlineGroup)}"]`);x.hidden=!x.hidden;});}
