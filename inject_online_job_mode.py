...
<section id="online-dashboard" class="online-dashboard" aria-live="polite">
  <section class="method online-method" aria-live="polite"><div><div class="eyebrow">ОНЛАЙН-СРЕЗ РЫНКА ТРУДА</div><div class="method-region" id="online-method-region">Выберите регион</div></div><div><div class="method-title" id="online-method-title">Доступность и оплата актуальных объявлений</div><div class="method-text" id="online-method-text">Индекс сопоставляет число объявлений с размером рабочей силы региона и уровнем предлагаемой зарплаты.</div></div></section>
  ...
  <section class="online-section"><h3>Как интерпретировать онлайн-индекс</h3><p class="online-note">Высокий балл означает, что относительно размера рабочей силы объявлений мало и/или предлагаемая зарплата ниже типичной для региона. Индекс сравнивает регионы между собой и не является вероятностью трудоустройства. Профессиональный рейтинг строится по выборке доступных карточек, поэтому его следует читать как индикатор, а не как полный учёт всех вакансий.</p><div class="online-downloads"><a href="online_job_vacancies_combined.csv" download>Вакансии, CSV</a><a href="online_job_market_by_region.csv" download>Показатели регионов, CSV</a><a href="online_job_market_by_region_and_group.csv" download>Профессиональные группы, CSV</a><a href="online_job_sources_status.csv" download>Покрытие источников, CSV</a></div></section>
</section>
'''
...
function renderOnlinePanel(iso){const d=onlineRegionData[iso];if(!d){onlineMethodRegion.textContent='Регион без сопоставимого среза';onlineMethodTitle.textContent='Онлайн-индекс не рассчитан';onlineMethodText.textContent='Отсутствие данных не означает, что искать работу здесь легче или труднее.';onlinePanel.innerHTML='<div class="eyebrow">Нет данных онлайн-сервисов</div><div class="region-name">Для региона нет сопоставимого среза</div><p class="online-note">Отсутствие данных не означает лёгкость или сложность поиска.</p>';return;}onlineSelected=iso;onlineMethodRegion.textContent=`${d.region} · ${d.score.toFixed(1)} из 10`;onlineMethodTitle.textContent=d.score>=7?'Сравнительно трудно найти объявление с приемлемой оплатой':d.score>=4?'Средняя относительная сложность поиска':'Сравнительно легче найти объявление с приемлемой оплатой';onlineMethodText.textContent=`На 10 тыс. человек рабочей силы приходится ${d.listings_per_10k.toFixed(1)} объявления; медианная предлагаемая зарплата — ${d.median_salary?onlineFmt.format(d.median_salary)+' ₽':'не указана'}.`;document.querySelectorAll('.online-region').forEach(x=>x.classList.toggle('selected',x.dataset.iso===iso));onlinePanel.innerHTML=`<div class="eyebrow">ОНЛАЙН-ОБЪЯВЛЕНИЯ · ${onlineEsc(d.snapshot_at.slice(0,10))}</div><div class="region-name">${onlineEsc(d.region)}</div><div class="overall"><span>Сложность найти приемлемое объявление</span><strong style="color:${onlineColor(d.score)}">${d.score.toFixed(1)}</strong></div><div class="online-metrics"><div><span>Найдено объявлений на площадках</span><strong>${onlineFmt.format(d.reported_total)}</strong></div><div><span>На 10 тыс. рабочей силы</span><strong>${d.listings_per_10k.toFixed(1)}</strong></div><div><span>Медиана зарплат выборки</span><strong>${d.median_salary?onlineFmt.format(d.median_salary)+' ₽':'нет данных'}</strong></div><div><span>Зарплата указана</span><strong>${d.salary_coverage.toFixed(0)}% выборки</strong></div></div><h3 class="rank-title" style="margin-top:15px">Группы, где выбор предложений слабее</h3><p class="online-note">Рейтинг основан на доступной выборке; малые группы статистически сглажены.</p>${d.groups.slice(0,9).map(g=>`<div class="online-rank"><b>${g.rank}</b><button class="rank-name" data-online-group="${onlineEsc(g.name)}">${onlineEsc(g.name)}<small>${g.sample} карточек · медиана ${g.median_salary?onlineFmt.format(g.median_salary)+' ₽':'не указана'}</small></button><span class="badge" style="background:${onlineColor(g.score)}">${g.score.toFixed(1)}</span></div><div class="online-example" data-online-detail="${onlineEsc(g.name)}" hidden>${g.examples.map(v=>`<a href="${onlineEsc(v.url)}" target="_blank" rel="noopener">${onlineEsc(v.title)}</a>${v.salary_mid_rur?' · '+onlineFmt.format(v.salary_mid_rur)+' ₽':''}`).join('<br>')||'Нет примеров в выборке'}</div>`).join('')}`;onlinePanel.querySelectorAll('[data-online-group]').forEach(b=>b.onclick=()=>{const x=onlinePanel.querySelector(`[data-online-detail="${CSS.escape(b.dataset.onlineGroup)}"]`);x.hidden=!x.hidden;});}
