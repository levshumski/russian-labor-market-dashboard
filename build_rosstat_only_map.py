...
def score_label(score):
    if score >= 8.5:
        return "Очень трудно найти работу с нормальной для региона зарплатой: вариантов мало, конкуренция высокая"
    if score >= 7:
        return "Трудно найти работу с нормальной для региона зарплатой: условия хуже, чем в большинстве регионов"
    if score >= 5:
        return "Найти работу с нормальной зарплатой скорее трудно"
    if score >= 3:
        return "Средние условия поиска работы по сравнению с другими регионами"
    return "Найти работу с нормальной для региона зарплатой сравнительно легче"
...
<div class="subtitle">Карта показывает, где сравнительно легче или труднее найти работу с зарплатой, типичной для региона. 1 — условия лучше, 10 — хуже. Основа — данные Росстата о занятости и вакансиях за 2024 год, зарплатах и прожиточном минимуме за 2025 год.</div>
...
<p class="guide-lead"><strong>Простыми словами:</strong> индекс сравнивает регионы и показывает, где человеку труднее найти работу с зарплатой, обычной для этого региона. Под такой работой проект понимает не «идеальную» зарплату, а вакансию с оплатой около региональной медианы или выше. Высокий балл означает, что вакансий относительно мало, людей в поиске много, нужные работодателям профессии хуже совпадают с профессиями жителей, поиск длится дольше и зарплата оставляет меньше денег сверх прожиточного минимума. Это сравнение регионов, а не вероятность трудоустройства конкретного человека.</p>
...
<article class="guide-card"><h3>Что означает оценка уровня зарплаты?</h3><p>Это сравнительный балл от 1 до 10 по сумме сверх прожиточного минимума. Например, при типичной зарплате 70 тыс. руб. и минимуме 20 тыс. руб. разница равна 50 тыс. руб. Это не реальные свободные деньги: показатель не знает стоимость жилья, состав семьи, налоги и личные расходы.</p></article>
...
function renderMethod(d){method.innerHTML=`<div><div class="eyebrow">УСЛОВИЯ ПОИСКА РАБОТЫ</div><div class="method-region">${esc(d.region)} · ${d.score.toFixed(1)} из 10</div></div><div><div class="method-title">${esc(d.interpretation)}</div><div class="method-text">Чем выше балл, тем меньше вакансий относительно числа людей, которым нужна работа, хуже совпадают профессии, дольше длится поиск или меньше типичная зарплата сверх прожиточного минимума. Возможная оценка при других весах зарплаты: ${d.score_min.toFixed(1)}–${d.score_max.toFixed(1)}. Последние доступные данные (${esc(d.current_labor_period)}): ${fmt(d.labor_force_thousand_current,1)} тыс. работающих или ищущих работу, безработица ${fmt(d.unemployment_rate_current,1)}%.</div></div>`;}
