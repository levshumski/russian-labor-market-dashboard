...
def score_label(score):
    if score >= 8.5:
        return "Очень высокая сложность поиска работы с типичным для региона уровнем оплаты: предложение вакансий ограничено, конкуренция среди соискателей повышена"
    if score >= 7:
        return "Высокая сложность поиска работы с типичным для региона уровнем оплаты: условия менее благоприятны, чем в большинстве регионов"
    if score >= 5:
        return "Сложность поиска работы несколько выше среднего уровня по регионам"
    if score >= 3:
        return "Условия поиска работы соответствуют среднему уровню по регионам"
    return "Сравнительно благоприятные условия поиска работы с типичным для региона уровнем оплаты"
...
# любую работу, а работу с типичной для региона зарплатой. 85% балла по-прежнему описывают
...
if pd.isna(validation_spearman):
    ...
    validation_summary = (
        f"Порядок регионов хорошо согласуется с официальным показателем напряжённости рынка труда "
        f"(коэффициент сходства {validation_spearman:.2f} из 1)."
    )
elif validation_spearman >= 0.40:
    validation_summary = (
        f"Порядок регионов умеренно согласуется с официальным показателем напряжённости рынка труда "
        f"(коэффициент сходства {validation_spearman:.2f} из 1)."
    )
else:
    validation_summary = (
        f"Порядок регионов слабо согласуется с официальным показателем напряжённости рынка труда "
        f"(коэффициент сходства {validation_spearman:.2f} из 1), поэтому оценку нужно трактовать осторожно."
    )
...
regional_export["Версия_формулы"] = (
    "4.1: 34% доля людей без работы или готовых работать + 30% нехватка вакансий + "
...
audit_rows = [
    ("Версия формулы", "4.1", "Условия поиска работы с типичной для региона зарплатой"),
    ...
    ("Республика Саха (Якутия): сложность трудоустройства", round(sakha_score, 2), "большая доля людей без работы или готовых работать и нехватка вакансий повышают балл"),
...
for row in regional_for_map.itertuples(index=False):
    ...
    map_data[row.shapeISO] = {
        ...
        "method_note": "34% — доля людей без работы или готовых работать; 30% — достаточно ли вакансий; 13% — совпадают ли профессии кандидатов и вакансий; 8% — как долго ищут работу; 15% — насколько типичная зарплата превышает прожиточный минимум.",
...
<title>Индекс условий поиска работы в регионах России</title>
...
<h1 class="title">Индекс условий поиска работы в регионах России</h1>
<div class="subtitle">Карта отражает сравнительные условия поиска работы с уровнем оплаты, соответствующим медианной заработной плате региона. Значение 1 соответствует наиболее благоприятным условиям, значение 10 — наименее благоприятным. Расчёт основан на данных Росстата о занятости и вакансиях за 2024 год, заработной плате и прожиточном минимуме за 2025 год.</div>
...
<section class="analytics" aria-labelledby="analytics-title"><div class="analytics-head"><div><h2 class="analytics-title" id="analytics-title">Зарплаты и условия поиска работы</h2><div class="analytics-note">Графики построены по тем же данным Росстата. Нажмите на регион, чтобы показать его на карте и в правой панели.</div></div></div><div class="salary-chart-wrap"><h3 class="chart-title">Типичная зарплата по регионам</h3><div class="chart-note">Показана медианная начисленная зарплата за апрель 2025 года. Медиана — середина распределения: половина работников получает меньше этой суммы, половина — больше.</div><div class="chart-controls" role="group" aria-label="Выбор регионов на диаграмме зарплат"><button type="button" class="chart-control is-active" data-salary-view="highest">20 самых высоких</button><button type="button" class="chart-control" data-salary-view="lowest">20 самых низких</button><button type="button" class="chart-control" data-salary-view="all">Все регионы</button></div><svg class="chart-svg" id="salary-chart" role="img" aria-label="Диаграмма типичной зарплаты по регионам России"></svg></div><div class="analytics-grid"><div class="analysis-plot"><h3 class="chart-title">Сумма сверх прожиточного минимума и условия поиска</h3><div class="chart-note">По горизонтали — разница между типичной зарплатой и прожиточным минимумом; по вертикали — индекс условий поиска работы. Это ориентир для сравнения регионов, а не сумма, которая обязательно останется у человека после расходов. Размер круга показывает численность населения.</div><svg class="chart-svg" id="relationship-chart" role="img" aria-label="Связь зарплаты и условий поиска работы"></svg></div><div class="analysis-plot"><h3 class="chart-title">Сколько регионов относится к каждому уровню сложности</h3><div class="chart-note">Столбцы показывают число регионов в каждом диапазоне индекса. Оранжевым выделён диапазон выбранного региона.</div><svg class="chart-svg" id="distribution-chart" role="img" aria-label="Распределение регионов по условиям поиска работы"></svg></div></div></section>
...
<h2 class="guide-title" id="guide-title">Что показывает индекс условий поиска работы</h2>
<p class="guide-lead"><strong>Назначение показателя:</strong> индекс сопоставляет субъекты Российской Федерации по условиям поиска работы с оплатой не ниже уровня, типичного для соответствующего региона. Типичным уровнем считается медианная заработная плата: половина работников получает меньше этой суммы, половина — больше. Высокая оценка указывает на относительную нехватку вакансий, повышенную конкуренцию, несоответствие профессиональной структуры спроса и предложения, более длительный поиск работы либо недостаточный уровень оплаты по отношению к прожиточному минимуму. Индекс характеризует региональный рынок труда в целом и не является оценкой вероятности трудоустройства конкретного человека.</p>
...
<article class="guide-card"><h3>Определение вакансии</h3><p>Под вакансией понимается один работник, необходимый организации для замещения свободного рабочего места, согласно сведениям обследованных Росстатом организаций.</p></article>
<article class="guide-card"><h3>Сколько людей нуждается в работе — 34%</h3><p>Учитываются официально безработные и люди, которые хотели бы работать, но перестали активно искать место. Чем больше их доля, тем выше возможная конкуренция за вакансии.</p></article>
<article class="guide-card"><h3>Достаточно ли открытых рабочих мест — 30%</h3><p>Считается, какая часть всех рабочих мест сейчас свободна. Чем меньше эта доля, тем меньше выбор у соискателя.</p><span class="plain-formula">Доля свободных мест = вакансии ÷ (занятые места + вакансии)</span></article>
<article class="guide-card"><h3>Совпадают ли профессии людей и вакансий — 13%</h3><p>Проект сравнивает девять крупных групп профессий среди работающих людей и среди вакансий. Если работодатели ищут специалистов не тех направлений, в которых занято большинство жителей, подходящую работу найти сложнее.</p></article>
<article class="guide-card"><h3>Как долго люди ищут работу — 8%</h3><p>Учитывается средняя продолжительность поиска и доля людей, которые ищут место год или дольше. Чем дольше поиск, тем сложнее рынок для соискателя.</p></article>
<article class="guide-card"><h3>Сколько зарплаты остаётся сверх базового минимума — 15%</h3><p>Из типичной зарплаты региона вычитается прожиточный минимум трудоспособного человека. Чем меньше разница, тем слабее зарплата покрывает расходы сверх самого базового уровня.</p></article>
<article class="guide-card"><h3>Оценка уровня заработной платы</h3><p>Это сравнительная оценка от 1 до 10, рассчитанная по разнице между медианной заработной платой и прожиточным минимумом. Например, при медианной заработной плате 70 тыс. руб. и прожиточном минимуме 20 тыс. руб. разница составляет 50 тыс. руб. Показатель не следует трактовать как фактически располагаемую сумму, поскольку он не учитывает стоимость жилья, состав семьи, налоги и индивидуальные расходы.</p></article>
<article class="guide-card guide-wide"><h3>Как пять показателей становятся числом от 1 до 10?</h3><ul><li>По каждому показателю регионы расставляются от лучших условий к худшим. Положение региона в этом списке переводится в число от 0 до 1 — это называется процентилем.</li><li>Так Москва не получает преимущество только из-за большого населения, а единичное необычно высокое значение не растягивает всю шкалу.</li><li>Пять оценок складываются с весами 34% + 30% + 13% + 8% + 15%, после чего итог переводится в понятную шкалу 1–10.</li><li>Рядом показывается возможный диапазон оценки, если немного изменить вес зарплаты. Широкий диапазон предупреждает, что точное место региона нестабильно.</li></ul><p>Это шкала порядка: балл 8 означает «хуже, чем у большинства регионов», но не означает, что искать работу ровно вдвое труднее, чем при балле 4.</p></article>
<article class="guide-card"><h3>Что означает оценка отдельной группы профессий?</h3><p>Она показывает, насколько трудно найти работу этого направления с зарплатой около типичной для региона или выше. Вакансия с низкой оплатой учитывается частично, а вакансия с оплатой не ниже региональной медианы — полностью. Высокая зарплата не увеличивает число вакансий искусственно.</p></article>
...
function ensureGrowthInflationChart(){
 ...
 medianHost.parentElement.insertAdjacentHTML("afterend",`<div class="forecast-line"><h3 class="chart-title">Рост зарплаты и рост цен</h3><div class="chart-note">Синяя линия показывает, на сколько процентов в среднем выросла зарплата без поправки на рост цен. Красная линия показывает инфляцию — средний рост цен за год. Если синяя линия выше красной, покупательная способность зарплаты в среднем увеличивается. Данные за 2021–2025 годы фактические; значения на 2026–${forecastMeta.cbr_table_last_year} годы — средний прогноз ${forecastMeta.cbr_respondents} экономистов из опроса Банка России ${forecastMeta.cbr_survey_dates}. Полупрозрачная область показывает основной диапазон мнений экспертов, поэтому прогноз не следует читать как точное обещание.</div><div class="growth-chart-legend"><span><i class="wage"></i>Рост зарплаты без поправки на цены</span><span><i class="inflation"></i>Средний рост цен</span><span><i class="wage-band"></i>Диапазон прогнозов зарплаты</span><span><i class="inflation-band"></i>Диапазон прогнозов инфляции</span><span>сплошная линия — известные данные · пунктир — прогноз</span></div><svg id="wage-inflation-chart" role="img" aria-label="Известные значения и прогнозы роста зарплаты и цен"></svg></div>`);
...
function forecastTable(rows,isOccupation=false,year=forecastYear){return `<table class="forecast-table"><thead><tr><th>№</th><th>${isOccupation?"Профессия":"Группа"}</th><th class="num">Ожидаемая востребованность</th><th class="num">Уровень зарплаты</th><th class="num">Общая оценка</th><th class="num">Зарплата</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${row.rank}</td><td><span class="forecast-name">${esc(row.name)}</span>${isOccupation?`<span class="forecast-parent">${esc(row.parent_group)}</span>`:""}<span class="forecast-parent">${esc(row.scenario_note||"")}</span></td><td class="num">${fmt(row.demand_score,1)} из 10<span class="forecast-parent">около ${fmt(row.projected_required_workers)} требуемых работников</span></td><td class="num">${fmt(row.salary_score,1)} из 10</td><td class="num"><span class="forecast-badge" style="background:${scoreColor(row.opportunity_score)}">${fmt(row.opportunity_score,1)}</span></td><td class="num"><span class="salary-comparison" title="${esc(row.salary_source||'')}"><span class="salary-current">${row.salary_base_year||2025}: ${fmt(row.current_salary_2025)} ₽${row.salary_observations?` · объявлений в выборке: ${fmt(row.salary_observations)}`:""}</span><span class="salary-forecast">${year}: ${fmt(row.predicted_salary)} ₽</span><span class="index-delta">рост без поправки на инфляцию ${signed(row.salary_growth_pct,1)}%</span></span></td></tr>`).join("")}</tbody></table>`;}
...
function renderCities(){const host=document.getElementById("city-explanations");host.innerHTML=Object.entries(cityJobExplanations).map(([city,data])=>`<article class="city-card"><h3>${esc(city)}</h3><div class="city-summary">Общая оценка условий поиска работы — <strong>${fmt(data.regional_score,1)} из 10</strong>: в целом найти работу здесь сравнительно легче. Люди без работы или готовые работать составляют ${fmt(data.broad_underutilization_rate,1)}%, профессии жителей и вакансий не совпадают на ${fmt(data.mismatch_pct,1)}%, а типичная зарплата превышает прожиточный минимум на ${fmt(data.salary_buffer)} ₽. При этом отдельные направления могут быть заметно сложнее общего рынка города.</div>${data.rows.map(row=>`<div class="city-row"><div class="city-row-head"><span>${esc(row.name)}</span><span style="color:${scoreColor(row.score)}">${fmt(row.score,1)} / 10</span></div><div>${cityReason(row)}</div><div class="forecast-parent">Примеры профессий внутри группы: ${row.examples.map(esc).join(", ")}.</div></div>`).join("")}</article>`).join("");}
...
function renderMethod(d){method.innerHTML=`<div><div class="eyebrow">УСЛОВИЯ ПОИСКА РАБОТЫ</div><div class="method-region">${esc(d.region)} · ${d.score.toFixed(1)} из 10</div></div><div><div class="method-title">${esc(d.interpretation)}</div><div class="method-text">Повышение оценки связано с сокращением числа вакансий относительно числа граждан, нуждающихся в работе, усилением профессионального несоответствия, увеличением продолжительности поиска либо снижением медианной заработной платы относительно прожиточного минимума. Диапазон оценки при изменении веса зарплатного показателя: ${d.score_min.toFixed(1)}–${d.score_max.toFixed(1)}. Последние доступные данные (${esc(d.current_labor_period)}): рабочая сила — ${fmt(d.labor_force_thousand_current,1)} тыс. человек, уровень безработицы — ${fmt(d.unemployment_rate_current,1)}%.</div></div>`;}
function renderPanel(d,pinned=false,preserve=false){
 ...
 const groupRows=visibleRows.map(({row:r,index})=>{
  ...
  const detail=openGroup===key?`<div class="group-detail"><strong>${esc(r.name)}</strong><br>Организации сообщили, что им требуется ${fmt(r.required_workers)} работников; занято ${fmt(r.staff_count)} человек. ${marketExplanation}<br>Оценочная зарплата группы: ${fmt(r.estimated_professional_wage)} руб.; сумма сверх прожиточного минимума: ${fmt(r.estimated_professional_wage_buffer)} руб.; оценка уровня зарплаты: ${fmt(r.salary_attractiveness_score,1)} из 10. Зарплата оценена по типичной зарплате региона и официальному соотношению зарплат профессиональных групп по России.<br>После статистической корректировки маленьких выборок свободны ${fmt(r.adjusted_vacancy_rate_pct,1)}% рабочих мест; с учётом оплаты — ${fmt(r.salary_adjusted_vacancy_rate_pct,1)}%.<br>На группу приходится ${fmt(r.staff_share_pct,1)}% занятых и ${fmt(r.vacancy_share_pct,1)}% вакансий региона. Возможный балл при других настройках: ${fmt(r.score_min,1)}–${fmt(r.score_max,1)}; место: ${fmt(r.rank_min)}–${fmt(r.rank_max)}. ${esc(r.observation_status)}. Маленькие группы приближаются к среднему по России: сила этой поправки обозначена κ = ${fmt(r.kappa_mom,0)}, а собственные данные региона влияют на результат на ${fmt(r.credibility_weight_pct,0)}%.</div>`:"";
 ...
 panel.innerHTML=`<div class="eyebrow">Регион · ${pinned?"закреплено":"наведение"}</div><div class="region-name">${esc(d.region)}</div><div class="overall"><span>Насколько трудно найти работу с типичной для региона зарплатой<br><small>1 — условия лучше, 10 — хуже среди регионов</small></span><strong style="color:${scoreColor(d.score)}">${d.score.toFixed(1)}</strong></div><div class="official-grid"><div><span>Типичная зарплата, апрель 2025</span><strong>${fmt(d.median_wage_2025)} руб.</strong></div><div><span>Оценка уровня зарплаты</span><strong>${fmt(d.salary_attractiveness_score,1)} из 10</strong></div><div><span>Сумма сверх прожиточного минимума</span><strong>${fmt(d.median_wage_buffer)} руб.</strong></div><div><span>Зарплата выше минимума в</span><strong>${fmt(d.median_wage_to_subsistence_ratio,2)} раза</strong></div><div><span>Свободные места среди всех рабочих мест</span><strong>${fmt(d.overall_vacancy_rate_pct,1)}%</strong></div><div><span>Люди без работы или готовые работать</span><strong>${fmt(d.broad_underutilization_rate,1)}%</strong></div><div><span>Среднее время поиска</span><strong>${fmt(d.average_job_search_months,1)} мес.</strong></div><div><span>Ищут год и дольше</span><strong>${fmt(d.long_term_unemployment_share,1)}%</strong></div><div><span>Насколько профессии людей и вакансий не совпадают</span><strong>${fmt(d.occupational_mismatch_pct,1)}%</strong></div></div><div class="rank-head"><label class="rank-search-label">Поиск по профессиям<input class="rank-search" type="search" value="${esc(filterQuery)}" placeholder="Например: программист" aria-label="Поиск профессиональной области или профессии"></label><div class="rank-title">Направления, в которых сложнее найти работу с нормальной зарплатой</div></div><div class="rank-subtitle">Рейтинг учитывает число вакансий и предлагаемую оплату. Очень маленькая для региона профессиональная группа показывается в поиске, но не получает автоматически худший балл из-за единичных наблюдений.</div>${rows}${empty}${more}<div class="legend"><div class="gradient-scale"><div class="gradient"></div><input class="gradient-slider" type="range" min="1" max="10" step="0.1" value="${selected.toFixed(1)}" aria-label="Выбрать индекс условий поиска работы"></div><div class="legend-labels"><span>1 · условия лучше</span><span>5 · средние</span><span>10 · условия хуже</span></div><div class="scale-readout"><span class="index-preview" style="background:${previewColor};color:${readableText(previewColor)}">Выбрано: ${selected.toFixed(1)}</span></div></div><div class="note">Вакансии и занятость — 2024 год; зарплата и прожиточный минимум — 2025 год. ${esc(d.coverage_note)} ${esc(d.denominator_stability)} ${esc(d.validation_summary)}</div>`;
...
function renderSalaryChart(){
 ...
 svg.append("g").selectAll("rect").data(data).join("rect").attr("class",d=>`salary-bar${d.iso===currentIso?" selected-bar":""}`).attr("x",x(0)).attr("y",d=>y(d.iso)).attr("width",d=>Math.max(1,x(d.median_wage_2025)-x(0))).attr("height",y.bandwidth()).on("mouseenter",(event,d)=>showChartTooltip(event,`<strong>${esc(d.region)}</strong>Типичная зарплата: ${rubles(d.median_wage_2025)}<br>Прожиточный минимум: ${rubles(d.working_age_subsistence_2025)}<br>Разница между зарплатой и минимумом: ${rubles(d.median_wage_buffer)}<br>Зарплата выше минимума в ${fmt(d.median_wage_to_subsistence_ratio,2)} раза<br>Условия поиска работы: ${fmt(d.score,1)} из 10`)).on("mousemove",moveChartTooltip).on("mouseleave",hideChartTooltip).on("click",(event,d)=>pinIso(d.iso));
...
function renderRelationshipChart(){
 ...
 svg.append("g").selectAll("circle").data(analyticsData.slice().sort((a,b)=>b.population_people-a.population_people)).join("circle").attr("class",d=>`scatter-point${d.iso===currentIso?" is-selected":""}`).attr("cx",d=>x(d.median_wage_buffer)).attr("cy",d=>y(d.score)).attr("r",d=>r(d.population_people)).on("mouseenter",(event,d)=>showChartTooltip(event,`<strong>${esc(d.region)}</strong>Типичная зарплата сверх прожиточного минимума: ${rubles(d.median_wage_buffer)}<br>Типичная зарплата: ${rubles(d.median_wage_2025)}<br>Прожиточный минимум: ${rubles(d.working_age_subsistence_2025)}<br>Зарплата выше минимума в ${fmt(d.median_wage_to_subsistence_ratio,2)} раза<br>Условия поиска работы: ${fmt(d.score,1)} из 10<br>Население: ${fmt(d.population_people)} чел.`)).on("mousemove",moveChartTooltip).on("mouseleave",hideChartTooltip).on("click",(event,d)=>pinIso(d.iso));
 ...
 svg.append("g").attr("class","chart-axis").attr("transform",`translate(0,${height-margin.bottom})`).call(d3.axisBottom(x).ticks(width<600?4:6).tickFormat(d=>`${fmt(d/1000,0)} тыс.`));svg.append("g").attr("class","chart-axis").attr("transform",`translate(${margin.left},0)`).call(d3.axisLeft(y).ticks(5));svg.append("text").attr("class","chart-axis-title").attr("x",(margin.left+width-margin.right)/2).attr("y",height-6).attr("text-anchor","middle").text("Типичная зарплата сверх прожиточного минимума, руб.");svg.append("text").attr("class","chart-axis-title").attr("transform","rotate(-90)").attr("x",-(margin.top+height-margin.bottom)/2).attr("y",14).attr("text-anchor","middle").text("Условия поиска работы: 1 — лучше, 10 — хуже");
}
...
const svg=d3.select(map).append("svg").attr("viewBox",`0 0 ${WIDTH} ${HEIGHT}`).attr("preserveAspectRatio","xMidYMid meet").attr("role","img").attr("aria-label","Карта условий поиска работы по регионам России");
...
# Единая официально-деловая терминология во всех динамических элементах.
formal_text_replacements = {
    "Насколько трудно найти работу с типичной для региона зарплатой": "Оценка условий поиска работы с типичным для региона уровнем оплаты",
    "1 — условия лучше, 10 — хуже среди регионов": "1 — наиболее благоприятные условия; 10 — наименее благоприятные",
    "Типичная зарплата": "Медианная заработная плата",
    "Оценка уровня зарплаты": "Оценка уровня заработной платы",
    "Люди без работы или готовые работать": "Доля безработных и потенциальной рабочей силы",
    "Насколько профессии людей и вакансий не совпадают": "Несоответствие структуры занятости и вакансий",
    "Направления, в которых сложнее найти работу с нормальной зарплатой": "Профессиональные группы с наименее благоприятными условиями трудоустройства",
    "Очень маленькая для региона профессиональная группа показывается в поиске, но не получает автоматически худший балл из-за единичных наблюдений.": "Малочисленные профессиональные группы доступны в поиске; единичные наблюдения не приводят к автоматическому присвоению наихудшей оценки.",
    "Показать ещё": "Показать следующие позиции",
    "условия лучше": "условия наиболее благоприятные",
    "условия хуже": "условия наименее благоприятные",
    "Нажмите на регион, чтобы показать его на карте и в правой панели.": "Выберите регион для его отображения на карте и в информационной панели.",
    "Что такое индекс и как он считается?": "Методика расчёта индекса",
}
for original_text, formal_text in formal_text_replacements.items():
    map_html = map_html.replace(original_text, formal_text)
